...
def upgrade() -> None:
    ...
    character_type_enum = sa.Enum('MAIN', 'SECONDARY', 'COLLECTIBLE', name='charactertype')
    character_type_enum.create(op.get_bind(), checkfirst=True)
    op.add_column('characters', sa.Column('character_type', character_type_enum, nullable=False))
    op.drop_constraint('characters_character_type_id_fkey', 'characters', type_='foreignkey')
...
def downgrade() -> None:
    ...
    op.add_column('characters', sa.Column('character_type_id', sa.INTEGER(), autoincrement=False, nullable=True))
    op.create_foreign_key('characters_character_type_id_fkey', 'characters', 'character_types', ['character_type_id'], ['id'])
