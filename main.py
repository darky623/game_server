    ...
    async with AsyncSessionFactory() as db:
        ...
        for character in user.characters:
            if character.character_type.name == 'main':
                ...
                response["character_info"] = CreateCharacterSchema(name=character.name,
                                                                   archetype_id=character.archetype_id)

        if not response["character_info"]:
            result = await db.execute(select(CharacterArchetype).where(CharacterArchetype.id == create_character.archetype_id))
            existing_character = result.scalars().first()
            if not existing_character:
                response["message"] = "There is no archetype with such an id!"
                return response

            character = Character(name=create_character.name,
                                  character_type_id=1,
